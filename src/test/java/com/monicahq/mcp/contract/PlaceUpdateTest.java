...
/**
 * Contract test for place_update MCP operation.
 * Tests updating existing places.
...
public class PlaceUpdateTest {
    ...
    @Test
    void shouldUpdatePlaceViaMcpProtocol() {
        // Given: MCP request to update a place
    ...
    @Test
    void shouldUpdatePlacePartialFields() {
        // Given: MCP request updating only some fields
        Map<String, Object> toolsCallRequest = Map.of(
            "jsonrpc", "2.0",
            "method", "tools/call",
            "params", Map.of(
        ...
        // Then: Should allow partial updates
        assertNotNull(response);
        assertEquals("2.0", response.get("jsonrpc"));
        assertTrue(response.containsKey("result"));
