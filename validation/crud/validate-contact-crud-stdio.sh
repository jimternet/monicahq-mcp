...
# MonicaHQ MCP Server - Contact CRUD Validation via STDIO
...
# Function to send MCP message via STDIO and capture response
send_mcp_message() {
    local message="$1"
    echo "$message" | MONICA_API_URL="http://localhost:8081/api" MONICA_API_TOKEN="$MONICA_API_TOKEN" java -jar build/libs/monicahqmcp-0.1.0.jar
}

echo "🧪 MonicaHQ MCP Server - Contact CRUD Validation (STDIO Mode)"
echo "============================================================"
...
log_info "Testing gender_list discovery tool via STDIO..."
...
CREATE_RESPONSE=$(send_mcp_message '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"contact_create","arguments":'$TEST_CONTACT_DATA'},"id":1}')
...
UPDATE_RESPONSE=$(send_mcp_message '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"contact_update","arguments":'$UPDATE_DATA'},"id":1}')
...
# Final Results
echo
echo "🎯 Contact CRUD Validation Results (STDIO Mode)"
echo "==============================================="
