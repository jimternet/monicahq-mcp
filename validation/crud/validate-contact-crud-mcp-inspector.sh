...
# MonicaHQ MCP Server - Contact CRUD Validation via MCP Inspector
...
echo "🧪 MonicaHQ MCP Server - Contact CRUD Validation (MCP Inspector)"
echo "==============================================================="
echo "Testing Constitutional Principle VII: Discovery Integration"
echo

log_info "Starting MCP inspector..."
npx @modelcontextprotocol/inspector --config mcp-config.json --server monicahq-mcp &
INSPECTOR_PID=$!

# Wait for inspector to start
sleep 10

# Function to cleanup MCP inspector
cleanup() {
    if [ ! -z "$INSPECTOR_PID" ]; then
        log_info "Stopping MCP inspector (PID: $INSPECTOR_PID)"
...
# Function to send MCP message via inspector proxy
send_mcp_message() {
    local message="$1"
    local proxy_url="http://localhost:6277"
    
    curl -s -X POST \
        -H "Content-Type: application/json" \
        -H "Authorization: Bearer 31a22fd0216574915bfa392f00aa8c122abb25adbe8344175ec912ad82093799" \
        -d "$message" \
        "$proxy_url" 2>/dev/null || echo '{"error": "request failed"}'
}

# Wait a bit more to ensure inspector is fully ready
sleep 5
...
if echo "$TOOLS_RESPONSE" | jq -e '.result.tools' > /dev/null 2>&1; then
    ...
    log_success "MCP tools list works - Found $TOOL_COUNT tools"
    ((TESTS_PASSED++))
    ...
    log_error "Response: $TOOLS_RESPONSE"
    ((TESTS_FAILED++))
    exit 1
fi
...
READ_RESPONSE=$(send_mcp_message '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"contact_get","arguments":{"id":"'$CREATED_CONTACT_ID'"}},"id":1}')
...
UPDATE_RESPONSE=$(send_mcp_message "$UPDATE_MESSAGE")
...
READ_AFTER_UPDATE=$(send_mcp_message '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"contact_get","arguments":{"id":"'$CREATED_CONTACT_ID'"}},"id":1}')
...
DELETE_RESPONSE=$(send_mcp_message '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"contact_delete","arguments":{"id":"'$CREATED_CONTACT_ID'"}},"id":1}')
...
READ_AFTER_DELETE=$(send_mcp_message '{"jsonrpc":"2.0","method":"tools/call","params":{"name":"contact_get","arguments":{"id":"'$CREATED_CONTACT_ID'"}},"id":1}')
...
# Final Results
echo
echo "🎯 Contact CRUD Validation Results (MCP Inspector)"
echo "================================================="
...
if [ $TESTS_FAILED -eq 0 ]; then
    ...
    echo
    echo "✅ Discovery Integration: gender_list tool works via MCP Inspector"
